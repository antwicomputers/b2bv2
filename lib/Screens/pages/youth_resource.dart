...
class YouthResource extends StatefulWidget {
  const YouthResource({super.key});

  @override
  State<YouthResource> createState() => _YouthResourceState();
}

class _YouthResourceState extends State<YouthResource> {
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              title: const Text('Youth Resource', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
              background: Container(
                ...
                child: const Center(
                  child: Opacity(
                    opacity: 0.12,
                    child: Icon(Icons.child_care, size: 100, color: _silver),
                  ),
                ),
              ),
            ),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const _SectionHeader(icon: Icons.info_outline, title: 'Submission Details'),
                    const SizedBox(height: 12),
                    _FormCard(
                      children: [
                        _StyledField(controller: _businessName, label: 'Program/Resource Name', icon: Icons.monetization_on, required: true),
                        _divider(),
                    ...
                    Consumer<UserProvider>(
                      builder: (context, value, child) => GestureDetector(
                        onTap: _isLoading
                            ...
                            : () async {
                                if (_image == null) {
                                  showSnackBar('Please add a resource photo', context);
                                  return;
                                }
                                if (!_formsKey.currentState!.validate()) return;
                                
                                setState(() => _isLoading = true);
                                String message = await value.youthRegister(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Center(
                            child: _isLoading ? const SizedBox(height: 22, width: 22, child: CircularProgressIndicator(strokeWidth: 2.5, color: Colors.black)) : const Text('Submit Youth Resource', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 0.5)),
                          ),
                        ),
                      ),
                    ),
